      ...
      {appwrite ? (
        <div className="flex flex-col gap-1.5">
          ...
          <img
            src={appwrite?.image}
            alt="hostelimg"
            className="w-full h-[70vh] max-sm:h-[300px]  object-cover rounded-md"
          />
          <span className="text-xs text-gray-600 font-semibold">
            Uploaded At: {new Date(appwrite?.$createdAt).toDateString()}
          </span>
          <span className="text-xs text-gray-600 font-semibold">
            Owned by: {appwrite?.ownedby}
          </span>
          <p className="text-xs text-gray-600 mt-1">
            {appwrite?.description || "-"}
          </p>
          <span className="text-xs text-gray-600 font-semibold">
            Rs {appwrite?.price}
          </span>
